...
@user_router.post(
    ...
    status_code=201,
    response_model=ProfessionalDataGet,
)
async def create_professional(
    ...
    try:
        new_profile = await services.create_professional_profile(
            session, user_id, professional_in
        )
        return new_profile
...
@user_router.patch(
    '/{user_id}/professional', response_model=ProfessionalDataGet
)
async def update_professional(
    ...
    update = await services.update_professional_data(
        session, user_id, professional_in
    )
