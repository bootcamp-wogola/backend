...
async def processar_checkin(
    ...
    if derivar_cvv:
        ...
        resposta_ia = await _gerar_sugestao(
            humor, payload.nota_semanal, payload.contexto
        )
...
async def _gerar_sugestao(
    humor: str, nota: int, contexto: str | None
) -> str | None:
    contexto_txt = f'Contexto adicional: {contexto}.' if contexto else ''

    prompt = (
        f'Você é um assistente de saúde mental empático e acolhedor do App BiT, '
