    ...
    def validate_type_specific_fields(self) -> "MentorshipBase":
        if self.type == MentorshipType.LIVE:
            ...
            if not self.streaming_link:
                raise ValueError("streaming_link is required for live mentorships")
            if not self.duration_minutes:
                raise ValueError("duration_minutes is required for live mentorships")

        elif self.type == MentorshipType.RECORDED:
            if not self.video_url:
                raise ValueError("video_url is required for recorded mentorships")
            if not self.video_duration_seconds:
                raise ValueError("video_duration_seconds is required for recorded mentorships")
